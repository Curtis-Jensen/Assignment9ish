...
public class HandTest extends JPanel {
	...
	private HandTest(PDeck deck, PHand hand) {
		...
		this.deck = deck; //They created their own deck and hand
		this.hand = hand;
		for (int i = 0; i < 4; i++)//Start of a for loop
			do {
				if (this.hand.getSize() > i)//if the hand is greater than the itteration of the for loop
					this.deck.addCard(this.hand.removeCard(i));
				deck.shuffle();//Well nothing's happening here.
